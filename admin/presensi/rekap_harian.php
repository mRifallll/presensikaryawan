...
if (empty($_GET['tanggal_dari'])) {
    ...
    $result = mysqli_query($con, "SELECT presensi.*, pegawai.nama, pegawai.lokasi_presensi FROM presensi JOIN pegawai ON presensi.id_pegawai = pegawai.id WHERE tgl_masuk = '$tanggal_hari_ini' ORDER BY tgl_masuk DESC");
} else {
    $tanggal_dari = $_GET['tanggal_dari'];
    $tanggal_sampai = $_GET['tanggal_sampai'];
    $result = mysqli_query($con, "SELECT presensi.*, pegawai.nama, pegawai.lokasi_presensi FROM presensi JOIN pegawai ON presensi.id_pegawai = pegawai.id WHERE tgl_masuk BETWEEN '$tanggal_dari' AND '$tanggal_sampai' ORDER BY tgl_masuk DESC");
}
...
<div class="page-body">
    <div class="container-xl">
        ...
        <?php if (empty($_GET['tanggal_dari'])) : ?>
            <span>Rekap Presensi Taggal : <?= date('d F Y') ?></span>
        <?php else : ?>
            <span>Rekap Presensi Taggal : <?= date('d F Y', strtotime($_GET['tanggal_dari']))  .  'Sampai'  .  date('d F Y', strtotime($_GET['tanggal_sampai'])) ?></span>
        <?php endif; ?>
